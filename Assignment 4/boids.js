...
class Particle {
	...
	step(){
        //List of positions of all neighbours (within radius of 100)
		...
		cohesion.div( 100 )  //scale (play with cohesion)
...
function draw(){
	...
	if (textoutput){
		...
		// let average = neighborDistances.reduce((accumulator, currentValue) => accumulator + currentValue) / Scene.swarm.length;
		outputDiv.innerText = neighborDistances.join('\n');
		// let formattedNeighborDistances = neighborDistances.map(subArray => subArray.map(num => num.toFixed(2).padStart(5, '0')));
		// outputDiv.innerText = formattedNeighborDistances.map(subArray => subArray.join('   ')).join('\n');
